...
import { TransactionWithAccount, formatNumber } from "./types";
      ...
      {transactions.length === 0 ? (
        <div className="alert alert-info m-3 d-flex align-items-center gap-2">
          ...
          <span>
            ...
            <Link href={`/transactions/${year}/${month}/new`} className="alert-link">
              En ajouter une
            </Link>
            .
          </span>
            ...
            <tbody>
              {transactions.map((tx) => (
                <tr key={tx.id}>
                  ...
                  <td className={`text-end small ${tx.type === "credit" ? "text-success fw-semibold" : "text-muted"}`}>
                    {tx.type === "credit" ? `+${formatNumber(tx.amount)} €` : "—"}
                  </td>
                  <td className={`text-end small ${tx.type !== "credit" ? "text-danger fw-semibold" : "text-muted"}`}>
                    {tx.type !== "credit" ? `−${formatNumber(tx.amount)} €` : "—"}
                  </td>
                  <td className="text-end" style={{ whiteSpace: "nowrap" }}>
                    <Link href={`/transactions/edit/${tx.id}`} className="btn btn-outline-primary btn-action me-1">
                      <i className="bi bi-pencil"></i>
                    </Link>
                    <button
                      className="btn btn-outline-danger btn-action"
                      onClick={() => window.confirm("Supprimer cette transaction ?")}
                    >
